...
// Collapse overlapping runs. This guards the common case — a double-clicked
// button or a retry hitting the same warm instance. It is per-instance and NOT
// a distributed lock, so it is a latency guard, not the correctness guarantee:
// correctness comes from the composite unique constraint that upsertRows writes
// against, which makes a concurrent duplicate run converge rather than double.
let inFlight = null;
...
// Newest sample_date already stored, across both tables the parser writes.
async function latestStoredDate(supabaseUrl, serviceKey) {
...
async function runSync({ from, to }, deps = {}) {
  ...
  let start = from;
  if (!start) {
    const newest = await latestStoredDate(supabaseUrl, serviceKey);
    // No history at all → pull the current harvest season from July 1.
    start = newest ? shiftDays(newest, -LOOKBACK_DAYS) : `${today.slice(0, 4)}-07-01`;
  }
  const end = to || today;
...
export default async function handler(req, res) {
  ...
  // A second caller joins the run already in flight instead of starting another.
  if (inFlight) {
    try {
