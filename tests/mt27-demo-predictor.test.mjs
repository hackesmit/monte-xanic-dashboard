...
test('MT.27 demo: predictor returns at least one card per expected reason', () => {
  ...
  try {
    ...
    // Note: riesgo-sobremadurez is now reachable — detectEdgeCase returns it when
    // yhat_brix is past brixUpper. When it co-occurs with an out-of-reach ANT
    // target the two are exposed together via prediction.flags (brixOverRipe /
    // antTargetUnreachable); reason keeps its precedence. Not asserted here.
